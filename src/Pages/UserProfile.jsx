...
const UserProfile = () => {
    ...
    function logout() {
        ...
        axios.post(env.BASE_URL + "/aps/user/logout", {}).then((res) => {
        ...
        <Page>
            <Card className="w-full mx-auto p-6 space-y-6">
                <div className="flex justify-between items-center -mb-2 ">
                    ...
                    <button className=" font-bold p-2 hover:bg-my-green-200   rounded-2xl relative"
                        ...
                        {isDropdownOpen && (
                            <>
                                ...
                                <div
                                    className="absolute top-10 right-0 bg-white text-green-900 shadow-lg rounded-lg p-2 w-44 z-10 text-sm">
                                    <button
                                        className=" w-full text-left px-4 py-2 hover:bg-my-green-100 flex items-center gap-3 opacity-50 cursor-not-allowed"
                                        disabled><PencilSimple size={20}/> Edit Profile
                                    </button>
                                    <button
                                        className=" w-full text-left px-4 py-2 text-yellow-600 hover:bg-amber-50 flex items-center gap-3"
                                        onClick={logout}><SignOut size={20}/> Logout
                                    </button>
                                    <button
                                        className=" w-full text-left px-4 py-2 text-red-800 text-nowrap hover:bg-rose-50 flex items-center gap-3"
                                        onClick={(e) => {
                                            e.preventDefault();
                                            setConfirmationDialog(true)
                                        }}><Trash size={20}/> Delete Profile
                                    </button>
                                </div>
                            </>
                        )}
            ...
            <div
                className={"h-full flex items-end justify-end fixed md:absolute top-0 left-0 w-full z-50  backdrop-blur-sm backdrop-filter backdrop-brightness-50 p-4 rounded-3xl " + (confirmationDialog ? 'flex' : 'hidden')}>

                <Card className={'bg-red-50 text-red-800 space-y-3   '}>
                    <h1 className="text-xl font-semibold tracking-tight   mb-3 text-center text-red-900">Delete
                        Account</h1>
                    ...
                    <div className="flex gap-5 item-end justify-end">
                        <button className="block rounded-md   text-red-500 hover:bg-red-100 px-3 py-2 "
                                onClick={deleteAccount}> Delete
                        </button>
                        <button
                            className="block rounded-md  bg-red-100 hover:bg-transparent text-red-800 hover:bg-red-100 px-3 py-2  "
                            onClick={() => setConfirmationDialog(false)}> Cancel
                        </button>
                    </div>
                </Card>
            </div>
